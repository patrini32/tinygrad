...
from tinygrad.helpers import cpu_objdump, capstone_flatdump, WIN
...
class ClangJITCompiler(Compiler):
  ...
  def compile(self, src:str) -> bytes:
    ...
    args = ['-march=native', '-O2', '-fno-math-errno', '-nostdlib', '-ffreestanding', '-nostartfiles', '-Wl,--oformat=binary']
    arch_args = ['-ffixed-x18'] if platform.machine() == 'arm64' else []
    os_args = ['-fPIC'] if not WIN else []
    with tempfile.NamedTemporaryFile(delete=True) as output_file:
      subprocess.check_output(['clang', '-x', 'c', *args, *arch_args, *os_args, '-', '-o', str(output_file.name)], input=src.encode('utf-8'))
      return pathlib.Path(output_file.name).read_bytes()
